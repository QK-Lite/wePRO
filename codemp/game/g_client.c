...
void ClientBegin( int clientNum, qboolean allowTeamReset ) {
	...
	if ( client->sess.sessionTeam != TEAM_SPECTATOR ) {
		...
		if (client->sess.sawMOTD == qfalse)
		{
			trap->SendServerCommand( ent-g_entities, va("print \"^5Hello, %s^5!\n^3[*] ^5Server is using ^7"GAMEVERSION_LONG" ^5v:^7"MOD_MAJOR"."MOD_MINOR" ("MOD_STAGE")\n\"", client->pers.netname) );	
			client->sess.sawMOTD = qtrue;
		}
	}
